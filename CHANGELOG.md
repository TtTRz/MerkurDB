...
- **`GET /v1/memories` browse endpoint** — filter-and-paginate listing of live memories for operators and UIs: `namespace` (query param overrides the `X-Merkur-Namespace` header bucket), `level` (comma list), `category`, `offset`, `limit` (default 20, clamped 1-200); returns `{items, total, offset, limit}` ordered newest-first. Soft-invalidated rows are excluded, matching every retrieval channel.
- **`/v1/status` gains `by_namespace`** — per-bucket memory counts alongside `by_level`, with the same counting semantics as `total_memories`; feeds the console's namespace distribution view.
- **Embedded read-only web console at `/ui`** — single-page observability UI compiled into the server binary (no build step, no separate deployment): status dashboard with level/namespace distribution, memory browse with filters + pagination and a detail pane, graph neighborhood explorer (force-directed canvas), and consolidation audit log. `/ui`, `/ui/app.js`, `/ui/style.css` are public; the console asks for a bearer token in the browser and reads everything through the authenticated API.
- **Consolidation log records write-governance counts** — `consolidate_log` gains `absorptions` and `invalidations` columns (schema v6, additive with 0 backfill), so the audit trail keeps the UPDATE/DELETE half of each tick alongside processed/edges/errors; `/v1/consolidate/log` and the console's log view expose them.
- **`/v1/graph/{id}` accepts a `namespace` query param** — bucket override with the same semantics as `/v1/memories` (query param wins over the `X-Merkur-Namespace` header).
- **`merkur-eval` benchmark harness** (`crates/eval`) — reproducible public-benchmark evaluation over the live HTTP serving path. LoCoMo track: per-turn ingest (date-prefixed, `dia_id` in context), LLM-free retrieval recall scored against evidence annotations, and judge-graded QA with an adversarial-aware judge (abstention scores correct on category 5). PersonaMem track (32k tier): judge-free multiple choice with in-situ checkpoint replay — each shared context accumulates into one namespace in ascending `end_index` order so no question sees future turns. Both tracks: retry-with-backoff on transient failures (per-question errors counted, never abort), concurrent execution (`--jobs`, plus `--context-jobs` for PersonaMem), per-question JSONL dumps, and progress logging. Orchestration: `scripts/fetch_locomo.sh`, `scripts/run_locomo.sh`, `scripts/run_personamem.sh` (throwaway server, parked schedulers, tagged reports).
...
- **Eval harness resilience + pipeline controls** — `pm-run --drain-consolidation` (answers wait for a settled consolidation queue), `pm-run --serve-abstracts` (answer model can consume distilled abstracts instead of raw turns), retry-with-backoff on chat calls (answer + judge), and a 180 s default chat timeout so a dead-but-open socket cannot freeze a multi-hour run. `scripts/run_personamem.sh` gains `MERKUR_EVAL_CONSOLIDATOR=llm` mode with dual endpoint preflight (answer model + consolidator are probed independently).

### Evaluation results (reference configuration)

- **LoCoMo**: QA accuracy 64.8% on raw-turn ingest, **68.3% with the consolidation pipeline enabled** (LLM abstracts + importance + edges; adjudication off) at retrieval depth 30 — retrieval hit@30 0.762 → 0.816, coverage 0.703 → 0.759; adversarial category 86.5-88.3%. **At depth 200 (guarded): 74.6% raw, 73.0% consolidated** — retrieval budget is the dominant lever, and the pipeline's composite re-rank gain is a shallow-pool phenomenon (its margin vanishes once top-200 already holds the evidence). Judge: `deepseek-v4-flash-vision-exp`.
- **PersonaMem 32k**: 73.2% across 589 questions on raw ingest, 0 unparseable answers; `kimi-k2.5` answering (same model family as the TencentDB Agent Memory reference, 76.1% with a full extraction pipeline). **With extraction + adjudication on: 71.2%** (348 adjudicated absorptions erased fact-recall evidence); **serving distilled abstracts to the answer model: 68.9%** (compression loses the verbatim detail that discriminates near-duplicate options). Raw-turn ingest + serving is the recommended shape for discriminating-choice workloads.
...
- **`mode=deep` serves its own vector seeds plus graph diffusion from them** — neighbor scores are anchored to the seed's relevance (`seed_score × edge-weight product × 0.5^depth`), so a weak seed's neighborhood can no longer outrank a strong seed's; a node reached both ways keeps the higher score.
- **Consolidation adjudication is faster and cheaper** — candidate sets with no pair above `adjudication_floor` skip the LLM call entirely (they could never execute), and verdict *gathering* (read-only: candidate search + LLM call) runs 4-wide while *execution* stays serial in pending order so two verdicts cannot race the same target.
- MCP `search_memory` tool upgraded from pure vector similarity to hybrid retrieval.

### Fixed

- **Deep search dropped its own vector seeds** — the BFS terminal select filtered `d > 0`, so deep mode returned only graph *neighbors* and discarded the vector hits anchoring relevance (LoCoMo hit@30 collapsed 0.81 → 0.17 on a consolidated corpus). Seeds are now merged with neighbors, deduped by max score.
- **`/v1/status` pending count excluded invalidation from its semantics** — absorbed (soft-invalidated) rows kept `pending_consolidation = 1` and were counted even though `list_pending` can never return them, so queue-drain waiters spun on phantoms. The stats query now matches `list_pending` (`invalid_at IS NULL`).
